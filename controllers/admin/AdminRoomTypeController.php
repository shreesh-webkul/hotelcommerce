 ...
 */
class AdminRoomTypeControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $modules_list = Hook::getHookModuleExecList('displayAdminRoomTypeExtra');
        ...
        if (Shop::isFeatureActive() && $this->context->cookie->id_category_products_filter) {
            ...
            if (!$category->inShop()) {
                $this->context->cookie->id_category_products_filter = false;
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminRoomType'));
            }
        }
    ...
    public function processPosition()
    {
        ...
        if (!Validate::isLoadedObject($object = $this->loadObject())) {
            ...
            $this->redirect_after = self::$currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&action=Customization&conf=5'.(($id_category = (Tools::getIsset('id_category') ? (int)Tools::getValue('id_category') : '')) ? ('&id_category='.$id_category) : '').'&token='.Tools::getAdminTokenLite('AdminRoomType');
        }
    }
            ...
            if ($this->ajax) {
                ...
                foreach ($this->available_tabs as $product_tab => $value) {
                    ...
                    $product_tabs[$product_tab] = array(
                        ...
                        'href' => $this->context->link->getAdminLink('AdminRoomType').'&id_product='.(int)Tools::getValue('id_product').'&action='.$product_tab,
                    );
                }
    ...
    public function renderKpis()
    {
        ...
        //     $helper->href = Context::getContext()->link->getAdminLink('AdminRoomType').'&productFilter_sav!quantity=0&productFilter_active=1&submitFilterproduct=1';
        ...
        if ($this->display == 'edit') {
            if (($product = $this->loadObject(true)) && $product->isAssociatedToShop()) {
                ...
                $js = (bool)Image::getImages($this->context->language->id, (int)$product->id) ?
                    'confirm_link(\'\', \''.$this->l('This will copy the images too. If you wish to proceed, click "Yes". If not, click "No".', null, true, false).'\', \''.$this->l('Yes', null, true, false).'\', \''.$this->l('No', null, true, false).'\', \''.$this->context->link->getAdminLink('AdminRoomType', true).'&id_product='.(int)$product->id.'&duplicateproduct'.'\', \''.$this->context->link->getAdminLink('AdminRoomType', true).'&id_product='.(int)$product->id.'&duplicateproduct&noimage=1'.'\')'
                    :
                    'document.location = \''.$this->context->link->getAdminLink('AdminRoomType', true).'&id_product='.(int)$product->id.'&duplicateproduct&noimage=1'.'\'';
                ...
                if ($this->tabAccess['delete']) {
                    $this->page_header_toolbar_btn['delete'] = array(
                        ...
                        'href' => $this->context->link->getAdminLink('AdminRoomType').'&id_product='.(int)$product->id.'&deleteproduct',
    ...
    public function renderForm()
    {
        ...
        $this->tpl_form_vars['form_action'] = $this->context->link->getAdminLink('AdminRoomType').'&'.($id_product ? 'updateproduct&id_product='.(int)$id_product : 'addproduct').($page > 1 ? '&page='.(int)$page : '');
    ...
    public function processBooking()
    {
        if (Tools::isSubmit('submitAddproductAndStay')) {
            ...
            $link = new Link();
            if ($checkTabClick) {
                Tools::redirectAdmin($link->getAdminLink('AdminRoomType').'&id_product='.$id_product.'&updateproduct&key_tab=Booking&date_from='.$date_from.'&date_to='.$date_to);
            }
        }
    }
    ...
    protected function _displaySpecificPriceModificationForm($defaultCurrency, $shops, $currencies, $countries, $groups)
    {
        ...
        $content .= '
			...
			<div class="panel-footer">
				<a href="'.$this->context->link->getAdminLink('AdminRoomType').($page > 1 ? '&submitFilter'.$this->table.'='.(int)$page : '').'" class="btn btn-default"><i class="process-icon-cancel"></i> '.$this->l('Cancel').'</a>
    ...
    {
        self::$currentIndex = 'index.php?tab=AdminRoomType';
        $product = new Product((int)Tools::getValue('id_product'));
        ...
        if ((bool)$obj->id) {
            if ($this->product_exists_in_shop) {
                ...
                $image_uploader->setMultiple(!(Tools::getUserBrowser() == 'Apple Safari' && Tools::getUserPlatform() == 'Windows'))
                    ...
                    Context::getContext()->link->getAdminLink('AdminRoomType').'&ajax=1&id_product='.(int)$obj->id
    ...
    public function initFormModules($obj)
    {
        ...
        $this->tpl_form_vars['custom_form'] = Hook::exec('displayAdminRoomTypeExtra', array(), (int)$id_module);
    }
    ...
    public function ajaxProcessPublishProduct()
    {
        if ($this->tabAccess['edit'] === '1') {
            if ($id_product = (int)Tools::getValue('id_product')) {
                $bo_product_url = dirname($_SERVER['PHP_SELF']).'/index.php?tab=AdminRoomType&id_product='.$id_product.'&updateproduct&token='.$this->token;
